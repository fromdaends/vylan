...
// The Instagram + X URLs are the clean canonical form; the founder's links
// carried QR-scan / share tracking (?igsh=…&utm_source=qr, ?s=11) that means
// nothing on a website.

type Social = {
  name: "LinkedIn" | "Instagram" | "X";
  // Accessible link label; falls back to `name`. X gets a clearer one than the
  // bare letter, which a screen reader would otherwise read ambiguously.
  label?: string;
  href: string;
...
export function SocialLinks({
  ...
  return (
    <div className={className}>
      {VYLAN_SOCIALS.map(({ name, label, href, Icon }) => (
        <a
          key={name}
          href={href}
          target="_blank"
