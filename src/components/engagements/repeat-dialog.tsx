...
export function RepeatDialog({
  ...
  // The invoice-recurrence SWITCH is gone (founder: it asked what the priced
  // lines already answer). setSeriesInvoiceRecreateAction survives as the
  // server-side writer — existing series keep whatever they were set to, and
  // nothing here starts charging a client because a control was removed.
  ...
  return (
    ...
    >
      ...
      <DialogContent className="max-h-[85vh] overflow-y-auto overflow-x-hidden sm:max-w-lg">
        ...
        {/* ── NO "recreate the invoice" SWITCH (founder ruling) ────────────
            It asked what the priced lines already answer — an occurrence
            carries this engagement's service lines and their timing, and
            bills through the ordinary acceptance path. What each occurrence
            will charge is worth STATING; making it a switch made it read as
            a second recurrence control sitting under the first. */}
        {series && series.status !== "ended" && invoiceSummary && (
