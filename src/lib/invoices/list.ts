...
export type InvoiceListResult = {
  ...
  page: number;
  hasNext: boolean;
  // True when migration 1310 hasn't reached this database: the list still
...
const WITH_1310_COLS = `${LEGACY_COLS}, amount_paid_cents, auto_chase, chase_count, last_chased_at`;
      ...
      paymentMethod:
        state.status === "paid"
          ? // The ledger's method when it has one; otherwise the rail on the
            // invoice, which is all a pre-1310 paid row can tell us.
