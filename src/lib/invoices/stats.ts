...
// payment ledger (migration 1270) is the right source — it is the only thing
// that knows about a partial payment, and the only thing with a real received
// date. But invoices settled BEFORE 1270 have no ledger rows at all. Counting
...
// 1270 is applied (every rail payment writes a ledger row from then on), so
...
export async function loadBillingStats(
  ...
  // pre-1270 fallback), and PostgREST's inferred row types are structurally
