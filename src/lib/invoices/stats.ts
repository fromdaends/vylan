...
// payment ledger (migration 1310) is the right source — it is the only thing
// that knows about a partial payment, and the only thing with a real received
// date. But invoices settled BEFORE 1310 have no ledger rows at all. Counting
...
// 1310 is applied (every rail payment writes a ledger row from then on), so
...
export async function loadBillingStats(
  ...
  // pre-1310 fallback), and PostgREST's inferred row types are structurally
