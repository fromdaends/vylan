...
- **A new database migration in `supabase/migrations/`:**
  ...
  3. Use that number + 10 for your new file (e.g., if highest is `0022`,
     ...
     Then check the number is actually free — this costs one command and
     stops a class of bug that cost months:

     ```
     ls supabase/migrations | sed 's/_.*//' | sort | uniq -d
     ```

     Must print nothing. Every parallel session computes the same
     "highest + 10" at the same moment, which is how eight pairs of files came
     to share a version. Supabase's ledger keys on the version, so the second
     file of a pair can never be tracked and a from-scratch replay cannot apply
     both — which is why the Supabase check failed on every migration PR for
     months (fixed in #1137). If your number collides, take the next free one.
  4. Tell the user what migration number you chose and why.
  5. Write the SQL idempotently (`if not exists` throughout), and make the code
     that reads it degrade or refuse cleanly while it is unapplied — a migration
     is live in the code before it is live in the database.
  6. **Give the founder the SQL to run, in a `sql` code block, in your summary.**
     That is the default and it never blocks.

  **NEVER STALL WAITING ON PERMISSION TO APPLY A MIGRATION.** Applying it
  yourself with `npx supabase@2.98.2 db push` is *allowed* if it goes through
  cleanly — the CLI on the founder's Mac is logged in and linked. But it is
  gated by the permission classifier, and if it is refused, or you are in a
  worktree that is not linked, or anything else gets in the way: **stop trying,
  hand the founder the SQL, and finish the task.** Never wait, never retry in a
  loop, never make a migration the reason a session is stuck. Shipping the code
  and handing over the SQL is a complete, correct outcome.
...
- Add new database columns, indexes, or tables via migration (following the multi-session migration-numbering rule). Applying it yourself is optional — if `db push` is blocked for any reason, hand the founder the SQL and move on. Never let a migration stall the task.
