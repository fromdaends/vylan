...
--   * The old firm-level UNIQUE constraints are replaced by uniqueness scoped to
--     (firm_id, client_id, ...) via `coalesce(client_id, <sentinel-uuid>)` unique
--     indexes. The sentinel (all-zeros uuid, which no real clients.id can equal)
--     collapses NULLs so a legacy firm-level row still can't duplicate, while
--     each real client gets its own independent uniqueness namespace.
...
--
-- Down (manual): drop the *_scope_qbo_idx / *_firm_client_idx indexes, restore the
-- original firm-level UNIQUE constraints, and drop the client_id columns.

-- Sentinel used to collapse NULL client_id in the unique indexes below. It is the
-- all-zeros UUID; clients.id is gen_random_uuid(), which never produces it.
...
-- per (firm, client), keeping at most one legacy firm-level (NULL) row per firm.
alter table quickbooks_connections
...
create index if not exists quickbooks_connections_client_idx
  on quickbooks_connections (firm_id, client_id);

-- ── 2. Cache tables: one row per (firm, client, qbo_id) ───────────────────────
-- Each client's QuickBooks has its OWN accounts / vendors / customers / tax codes
-- / items, so the cache must be scoped per client, not per firm.
...
create unique index if not exists quickbooks_accounts_scope_qbo_idx
  on quickbooks_accounts (
    firm_id,
    coalesce(client_id, '00000000-0000-0000-0000-000000000000'::uuid),
    qbo_id
  );
create index if not exists quickbooks_accounts_client_idx
  on quickbooks_accounts (firm_id, client_id);
...
create unique index if not exists quickbooks_vendors_scope_qbo_idx
  on quickbooks_vendors (
    firm_id,
    coalesce(client_id, '00000000-0000-0000-0000-000000000000'::uuid),
    qbo_id
  );
create index if not exists quickbooks_vendors_client_idx
  on quickbooks_vendors (firm_id, client_id);
...
create unique index if not exists quickbooks_customers_scope_qbo_idx
  on quickbooks_customers (
    firm_id,
    coalesce(client_id, '00000000-0000-0000-0000-000000000000'::uuid),
    qbo_id
  );
create index if not exists quickbooks_customers_client_idx
  on quickbooks_customers (firm_id, client_id);
...
create unique index if not exists quickbooks_tax_codes_scope_qbo_idx
  on quickbooks_tax_codes (
    firm_id,
    coalesce(client_id, '00000000-0000-0000-0000-000000000000'::uuid),
    qbo_id
  );
create index if not exists quickbooks_tax_codes_client_idx
  on quickbooks_tax_codes (firm_id, client_id);
...
create index if not exists quickbooks_items_client_idx
  on quickbooks_items (firm_id, client_id);
...
create unique index if not exists quickbooks_learned_scope_idx
  on quickbooks_learned_mappings (
    firm_id,
    coalesce(client_id, '00000000-0000-0000-0000-000000000000'::uuid),
    signal_type,
    source_key
  );
create index if not exists quickbooks_learned_client_idx
  on quickbooks_learned_mappings (firm_id, client_id);
